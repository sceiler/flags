...
Use the `getProviderData` function in your [Flags Discovery Endpoint](https://vercel.com/docs/flags/flags-explorer/getting-started#creating-the-flags-discovery-endpoint) to load and emit your Statsig data. Accepts an `options` object with the following keys.
